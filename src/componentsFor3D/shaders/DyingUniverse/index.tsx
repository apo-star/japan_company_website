...
export const DyingUniverse = () => {
  const fragementShader = `
        ...
        vec4 Star(ray r, float seed) {
            vec4 noise = Noise4(vec4(seed, seed + 1., seed + 2., seed + 3.));  // cache noise
            float t = fract(time * 0.1 + seed) * 2.;

            float fade = smoothstep(2., 0.5, t);  // use smoothstep for fading effect
        ...
        vec4 Ground(ray r) {
            ...
            for(int i=0; i<NUM_STARS; i++) {
                ...
                float lambert = clamp(dot(L, up), 0., 1.);
                ...
                #ifdef FLOOR_REFLECT
                float spec = pow(clamp(dot(R, L), 0., 1.), 400.);
                float fresnel = 1.-clamp(dot(L, up), 0., 1.);
  ...
  const [uniforms] = useState({
    ...
    iResolution: {
      type: "v2",
      value: new THREE.Vector2(100, 50),
    },
  });
